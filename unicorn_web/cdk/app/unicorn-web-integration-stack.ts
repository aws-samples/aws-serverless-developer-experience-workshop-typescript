...
export class WebIntegrationStack extends Stack {
  constructor(scope: Construct, id: string, props: WebIntegrationStackProps) {
    ...
    const propertiesEventBusArn = ssm.StringParameter.valueFromLookup(
      this,
      props.propertiesEventBusArnParam
    );
     ...
     */
    const propertiesEventBus = events.EventBus.fromEventBusArn(
      this,
      `propertiesEventBus-${props.stage}`,
      Lazy.string({ produce: () => propertiesEventBusArn })
    );
    ...
    new events.Rule(this, 'unicorn.web-PublicationEvaluationCompleted', {
      ...
      enabled: true,
      targets: [new targets.EventBus(props.webEventBus)],
    });
  }
}
