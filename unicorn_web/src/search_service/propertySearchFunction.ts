...
import { DynamoDBClient, GetItemCommand, GetItemCommandInput, QueryCommandInput, Condition, QueryCommand, QueryCommandOutput } from '@aws-sdk/client-dynamodb';
...
const SERVICE_NAME = process.env.POWERTOOLS_SERVICE_NAME ?? "propertySearch";
...
const PROJECTION_PROPERTIES = "country, city, street, contract, #num, description, listprice, currency, #status";
...
class PropertySearchFunction implements LambdaInterface {
    ...
    public async handler(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
        ...
        } else if (event.resource == "/properties/{country}/{city}/{street}/{number}") {
    ...
    private async listPropertiesByCity(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
        const country = event.pathParameters?.country;
        const city = event.pathParameters?.city;
        logger.info(`List properties by city: country = ${country}; city = ${city}`);
        ...
        if (data.Items === undefined) {
            return {
                statusCode: 400,
                body: JSON.stringify({message: `Unable to find results for ${event.resource}`})
            }
        } else {
            return {
                statusCode: 200,
     ...
     */
     @tracer.captureMethod()
    private async listPropertiesByStreet(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
        const country = event.pathParameters?.country;
        const city = event.pathParameters?.city;
        const street = event.pathParameters?.street;
        logger.info(`List properties by street: country = ${country}; city = ${city}; street = ${street}`);
     ...
     */
     @tracer.captureMethod()
    private async propertyDetails(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
        const country = event.pathParameters?.country;
        const city = event.pathParameters?.city;
        const street = event.pathParameters?.street;
        ...
        logger.info(`Get property details for: country = ${country}; city = ${city}; street = ${street}; number = ${number}`);
        ...
        if (data.Item === undefined) {
            return {
                statusCode: 404,
                body: JSON.stringify({message: `No property for ${JSON.stringify(event.pathParameters)}`})
            }    
        } else {
            const dataObj = unmarshall(data.Item) as PropertyDBType;
            if (dataObj.status !== `APPROVED`) {
                logger.warn(`Property for ${JSON.stringify(event.pathParameters)} is NOT APPROVED`);
                return {
                    statusCode: 404,
                    body: JSON.stringify({message: `No property for ${JSON.stringify(event.pathParameters)}`})
                }    
