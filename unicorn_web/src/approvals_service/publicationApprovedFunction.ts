...
const SERVICE_NAME = process.env.SERVICE_NAME ?? "PublicationApproved";
...
class PublicationApprovedFunction implements LambdaInterface {
    ...
    public async handler(event: EventBridgeEvent<string, PublicationEvaluationCompleted>, context: Context): Promise<void> {
        logger.info(`Property status changed: ${JSON.stringify(event.detail)}`);
        try {
            // Construct the entry to insert into database. 
            let propertyEvaluation: PublicationEvaluationCompleted = Marshaller.unmarshal(event.detail, 'PublicationEvaluationCompleted');
    ...
    private async savePropertyStatus(propertyEvaluation: PublicationEvaluationCompleted) {
        logger.info(`Updating status: ${propertyEvaluation.evaluationResult} for ${propertyEvaluation.propertyId}`);
        ...
        logger.info(`Updating database: ${propertyEvaluation.evaluationResult} for ${propertyEvaluation.propertyId}`);
        await this.updatePropertyFor(PK, SK, propertyEvaluation.evaluationResult);
        logger.info(`Updated database: ${propertyEvaluation.evaluationResult} for ${propertyEvaluation.propertyId}`);
    }
