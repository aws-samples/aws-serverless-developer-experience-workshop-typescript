...
describe("Unit tests for contract update", function () {
  ...
  test("Verifies successful response", async () => {
    ...
    expect([
      responseObj.contract_id,
      responseObj.property_id,
      responseObj.contract_status,
    ]).toEqual([
      contractId,
      "4781231c-bc30-4f30-8b30-7145f4dd1adb",
      "APPROVED",
    ]);
  });
  ...
  test("Invalid payload", async () => {
    ...
    expect(result.body).toEqual(
      JSON.stringify({ message: `Must specify property id` })
    );
  });
  ...
  test("No contract exists", async () => {
    ...
    expect(result.statusCode).toEqual(400);
    expect(result.body).toEqual(
      JSON.stringify({ message: `No contract found for specified Property ID` })
    );
  });
