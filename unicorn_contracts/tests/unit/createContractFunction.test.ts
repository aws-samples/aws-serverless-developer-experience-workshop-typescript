...
describe('Unit tests for contract creation', function () {
    ...
    test('verifies successful response', async () => {
        ...
        ddbMock.callsFake((input) => {
            let inputCommand = input as PutItemCommandInput
    ...
    test('Invalid payload', async () => {
        ...
        expect(result.body).toEqual(JSON.stringify({message: `Must specify contract details`}));
    });
    ...
    test('Failed database update', async () => {
        ...
        expect(result.body).toEqual(JSON.stringify({error: `Unable to create contract ${contractId}`}));
    });
    ...
    test('Failed event fire', async () => {
        ...
        const result = await lambdaHandler(event, context);
        expect(result.statusCode).toEqual(500);
        expect(result.body).toEqual(JSON.stringify({error: `Unable to fire 'Contract created' event for contract ${contractId}`}));
    });
});