...
import PropertyData from "../data/contract_data.json";
...
export async function* getCloudWatchLogsValues(propertyId: string): AsyncGenerator<any, void, unknown> {
    const groupName = (await findOutputValue('UnicornContractsCatchAllLogGroupArn')).split(":").slice(-2)[0];
...
export const initialiseDatabase = async () => {
    ...
    const command = new BatchWriteCommand({
        RequestItems: {
            [tableName]: PropertyData.map((property) => ({ PutRequest: { Item: property } }))
        }
    });
...
export const findOutputValue = async (outputKey: string) => {
    ...
    const stackResources: DescribeStacksCommandOutput = await cloudformation.send(new DescribeStacksCommand({ StackName: "uni-prop-local-contracts" }));
    if (stackResources.Stacks === undefined || stackResources.Stacks?.length < 1)
        throw new Error("Could not find stack resources named: uni-prop-local-contracts ");

    if (stackResources.Stacks[0].Outputs === undefined || stackResources.Stacks[0].Outputs?.length < 1) {
        throw new Error("Could not find stack outputs for stack named: uni-prop-local-contracts");
    }
