...
const SERVICE_NAME = process.env.SERVICE_NAME ?? "contractStatusChanged";
...
class ContractStatusChangedFunction implements LambdaInterface {
    ...
    public async handler(event: EventBridgeEvent<string, ContractStatusChanged>, context: Context): Promise<void> {
        logger.info(`Contract status changed: ${JSON.stringify(event.detail)}`);
        try {
            // Construct the entry to insert into database. 
            let statusEntry: ContractStatusChanged = Marshaller.unmarshal(event.detail, 'ContractStatusChanged');
    ...
    private async saveContractStatus(statusEntry: ContractStatusChanged) {
        logger.info(`Updating status: ${statusEntry.contractStatus} for ${statusEntry.propertyId}`);
        const ddbUpdateCommandInput: UpdateItemCommandInput = {
            TableName: DDB_TABLE,
            Key: { property_id: { S: statusEntry.propertyId } },
            UpdateExpression: 'set contract_id = :c, contract_status = :t, contract_last_modified_on = :m',
        ...
        logger.info(`Updated status: ${statusEntry.contractStatus} for ${statusEntry.propertyId}`);
