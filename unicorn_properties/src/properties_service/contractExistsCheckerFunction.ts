...
export class ContractStatusNotFoundException extends Error {
    ...
    constructor(message?: string | `No contract found for specfieid Property ID`) {
        super(message);
...
class ContractExistsCheckerFunction implements LambdaInterface {
    ...
    public async handler(event: any, context: Context): Promise<StepFunctionsResponse> {
        ...
        try {
            ...
            // No item? No contract?
            if (currentStatus === undefined || currentStatus.contract_id === undefined || currentStatus.contract_id === '') {
        ...
        if (!contractExists)
            throw new ContractStatusNotFoundException();
        else {
    ...
    private async getContractStatus(propertyId: string): Promise<ContractStatus | undefined> {
