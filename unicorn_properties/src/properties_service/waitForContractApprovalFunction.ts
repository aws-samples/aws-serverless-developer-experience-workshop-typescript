...
import { DynamoDBClient, GetItemCommand, GetItemCommandInput, UpdateItemCommand, UpdateItemCommandInput, UpdateItemCommandOutput } from '@aws-sdk/client-dynamodb';
...
const SERVICE_NAME = process.env.SERVICE_NAME ?? "contractExistsChecker";
...
class ContractStatusCheckerFunction implements LambdaInterface {
    ...
    public async handler(event: any, context: Context): Promise<StepFunctionsResponse> {
        ...
        try {
            ...
            let currentStatus = await this.getContractStatusItemFor(propertyId);
            ...
            logger.error(`Error during Contract Status Check: ${JSON.stringify(error)}`);
            return {
                statusCode: 500,
    ...
    private async getContractStatusItemFor(propertyId: string): Promise<ContractStatus | undefined> {
        const getItemCommandInput: GetItemCommandInput = {
            Key: {'property_id': {S: propertyId}},
            ProjectionExpression: 'contract_id, property_id, contract_status, sfn_wait_approved_task_token',
            TableName: DDB_TABLE
        };
