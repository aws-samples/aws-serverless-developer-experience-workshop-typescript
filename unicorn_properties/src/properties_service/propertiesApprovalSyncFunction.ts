...
import { Context, DynamoDBStreamEvent, AttributeValue, DynamoDBBatchResponse } from 'aws-lambda';
import { LambdaInterface } from '@aws-lambda-powertools/commons';
import { Metrics } from '@aws-lambda-powertools/metrics';
import { Logger } from '@aws-lambda-powertools/logger';
import { Tracer } from '@aws-lambda-powertools/tracer';
import { SFNClient, SendTaskSuccessCommand, SendTaskSuccessCommandInput } from '@aws-sdk/client-sfn';

// Set up the globals
const SERVICE_NAMESPACE = process.env.SERVICE_NAMESPACE ?? "unicorn.properties";
const SERVICE_NAME = process.env.SERVICE_NAME ?? "propertiesApprovalSync";
...
class PropertiesApprovalSyncFunction implements LambdaInterface {
    ...
    public async handler(event: DynamoDBStreamEvent, context: Context): Promise<DynamoDBBatchResponse> {
        logger.info(`DynamodDB stream processing triggered ${JSON.stringify(event)}`);
        // Track results.
        ...
        for (let i = 0; i < event.Records.length; i++) {
            ...
            const newImage: ContractStatus | undefined = this.unmarshallContractStatus(record.dynamodb?.NewImage);
            const oldImage: ContractStatus | undefined = this.unmarshallContractStatus(record.dynamodb?.OldImage);
            try {
                ...
                if (newImage.sfn_wait_approved_task_token !== undefined) {
                    ...
                    if (newImage.contract_status === 'APPROVED') {
                        ...
                        logger.info(`Sending CONTRACT APPROVED task success to token ${mergedImage.sfn_wait_approved_task_token}`);
                        await this.sendTaskSuccess(mergedImage.sfn_wait_approved_task_token);
                        logger.info(`Sent task success to token ${mergedImage.sfn_wait_approved_task_token}`);
                    } else {
                        logger.warn(`Contract ${mergedImage.contract_id} is not in APPROVED state. Skipping`);
                    }
                } else {
                    // Nothing to do, just an updated contract with no property approval workflow yet.
                    logger.info(`Contract ${mergedImage.contract_id} has no property approval requested yet`);
                }    
            } catch (error: any) {
                logger.error(`Failure during handling of event ${i}: ${JSON.stringify(record)} - error was ${JSON.stringify(error)}`);
                // Save the contract ID that caused the error.
                results.batchItemFailures.push({itemIdentifier: newImage?.contract_id ?? `Index: ${i}`});
            }
        }
     ...
     * @returns 
     */
    private unmarshallContractStatus(input: { [key: string]: AttributeValue } | undefined): ContractStatus | undefined {
        if (input === undefined) {
            return undefined;
        }
        ...
        if (input['sfn_wait_approved_task_token'] !== undefined) {
            result.sfn_wait_approved_task_token = input['sfn_wait_approved_task_token'].S;
        }
