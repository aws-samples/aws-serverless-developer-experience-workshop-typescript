...
export class PropertiesIntegrationStack extends Stack {
  constructor(scope: Construct, id: string, props: PropertiesIntegrationStackProps) {
    super(scope, id, props);


    // Integration with Web service
    const webEventBusArn = ssm.StringParameter.valueFromLookup(this, props.webEventBusArnParam)
    /**
     ...
     */
    const webEventBus = events.EventBus.fromEventBusArn(this, `webEventBus-${props.stage}`, Lazy.string({ produce: () => webEventBusArn}))
    ...
    new events.Rule(this, 'unicorn.properties-PublicationApprovalRequested', {
      ...
      description: 'Subscription to PublicationApprovalRequested events by the Properties Service.',
      ...
      enabled: true,
      targets: [
        new targets.EventBus(props.propertiesEventBus)
      ]
    })

    // Integration with Contracts service
    const contractsEventBusArn = ssm.StringParameter.valueFromLookup(this, props.contractsEventBusArnParam)
    /**
     *  When using valueFromLookup an initial dummy-value is returned prior to the lookup being performed using AWS APIs during synthesis. 
     *  As this property supports tokens, we convert the parameter value into a token to be resolved after the lookup has been completed using Lazy.
     */
    const contractsEventBus = events.EventBus.fromEventBusArn(this, 'event-bus', Lazy.string({ produce: () => contractsEventBusArn}))
    new events.Rule(this, 'unicorn-properties-ContractStatusChanged', {
      ruleName: 'unicorn.properties-ContractStatusChanged',
      description: 'Subscription to ContractStatusChanged events by the Properties Service.',
