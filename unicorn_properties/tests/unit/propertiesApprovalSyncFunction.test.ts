...
describe('Unit tests for contract creation', function () {
    ...
    test('verifies Approved check', async () => {
        ...
        function verifyTaskSend(input: any) {
            let cmd = input as SendTaskSuccessCommandInput;
            let taskToken = cmd.taskToken;
        ...
        const response: DynamoDBBatchResponse = await lambdaHandler(baselineDynamoDBEvent, context);
        expect(response.batchItemFailures.length).toEqual(0);
    });

    test('verifies Unapproved check', async () => {
        baselineDynamoDBEvent.Records[0].dynamodb.NewImage.contract_status.S = "New";
    ...
    test('verifies non-status update check', async () => {
        baselineDynamoDBEvent.Records[0].dynamodb.OldImage.contract_id.S = 'oldcontract1';
        baselineDynamoDBEvent.Records[0].dynamodb.NewImage.contract_status.S = 'Draft';
    ...
    test('verifies no task token check', async () => {
        ...
        const response: DynamoDBBatchResponse = await lambdaHandler(noTaskTokenEvent, context);
        // Expect no errors. 
        expect(response.batchItemFailures.length).toEqual(0);
    });
